...
def handle_follow(event):
    ...
    # ✏️ CHANGED: เปลี่ยนข้อความเป็น "ชื่อเล่น"
    line_bot_api.reply_message(
        event.reply_token,
        TextSendMessage(
            text="สวัสดีค่ะ! ยินดีต้อนรับสู่ผู้ช่วย DPIS6 กรุณาพิมพ์ **ชื่อเล่น** ของคุณเพื่อเริ่มการลงทะเบียนค่ะ"
        )
    )
...
def handle_message(event):
    ...
    if user_message.startswith("DEPT_CONFIRM:"):
        ...
        line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(
                text=f"✅ ลงทะเบียนสำเร็จ! คุณจาก {department_name} ยินดีให้บริการด้านโปรแกรม DPIS6 ค่ะ"
            )
        )
    ...
    if not user_profile or user_profile.get('isVerified') == False:
        ...
        if current_state == 'not_started':
            ...
            update_user_profile(user_id, {'state': 'waiting_for_name', 'isVerified': False})
            # ✏️ CHANGED: เปลี่ยนข้อความเป็น "ชื่อเล่น"
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text="ระบบไม่พบข้อมูลการลงทะเบียนของคุณ กรุณาพิมพ์ **ชื่อเล่น** เพื่อเริ่มการลงทะเบียนค่ะ"))
            
        elif current_state == 'waiting_for_name':
            # 3.1 ขั้นตอน: บันทึกชื่อ
            # ✏️ CHANGED: ปรับเงื่อนไขการตรวจสอบความยาวชื่อและข้อความแจ้งเตือน
            if len(user_message.strip()) > 0: # แค่ต้องไม่เป็นค่าว่าง
                ...
                line_bot_api.reply_message(
                 ...
                 line_bot_api.reply_message(event.reply_token, TextSendMessage(text="กรุณาพิมพ์ชื่อเล่นที่ต้องการใช้งานค่ะ"))
