...
# --- ข้อมูลกรม/ส่วนราชการ (ต้องมี 173 รายการ) ---
...
THAI_DEPARTMENTS = [
    ...
    "สำนักงบประมาณ",
    # 📌 กรุณาใส่ชื่อกรมทั้งหมด 173 รายการของคุณที่นี่
]
# -----------------------------------------------
...
def create_department_quick_replies(departments):
    ...
    for dept in departments[:13]: 
        # เมื่อผู้ใช้คลิก ปุ่มจะส่งข้อความ "DEPT_CONFIRM:ชื่อกรม" กลับมา
        items.append(
            QuickReplyButton(
                action=MessageAction(
                    label=dept,
                    # ใช้ Prefix ในการยืนยันเพื่อให้โค้ดรู้ว่านี่คือการเลือกกรม
                    text=f"DEPT_CONFIRM:{dept}" 
                )
            )
        )
...
def handle_message(event):
    ...
    # 3. 💡 CHECK STATUS: ถ้ายังไม่ยืนยันตัวตน
    if not user_profile or user_profile.get('isVerified') == False:
        
        current_state = user_profile.get('state')
        ...
        if current_state == 'waiting_for_name':
            ...
            # สถานะไม่ชัดเจน ให้เริ่มกระบวนการใหม่
             update_user_profile(user_id, {'state': 'waiting_for_name', 'isVerified': False})
